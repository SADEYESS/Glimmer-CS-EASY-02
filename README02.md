...
1）随机存取允许直接访问任意位置的数据项，不需要遍历或搜索数据结构中除了目标数据以外的多余元素。随机存取的时间复杂度接近常数时间O(1)

2）随机存取主要属于线性表结构中的顺序表（又被称作数组）的特征

#### **Q2：什么是指针？如何在C语言中定义指针变量？指针变量的大小是固定的吗？**
1）指针是存储内存地址的变量。它并不直接存储数据的值，而是存储某个特定对象在计算机内存中的位置，程序可以通过指针直接访问和修改存储在该内存地址中的数据。

2）要在Ｃ语言中定义一个指针变量，需要在最前面声明指针指向的数据类型，然后写出变量名并在前面加上星号。假设要定义不同类型的指针变量p，可以做以下声明：
```c
  int* p;  // 整数类型的指针p
 char* p;  // 字符类型的指针p
float* p;  // 浮点类型的指针p
...
3）在确定的系统环境下指针变量的大小是固定的，比如32位系统上指针的大小为4字节，64 位系统上指针的大小为8字节
...
根据这个性质，图可以用在不同站点之间的火车、汽车等交通工具的管理系统上
...
typedef struct VertexNode       //创建顶点节点的结构体，同样简化名称
{
    ...
    EdgeNode* first;            //创建从顶点节点指向第一个边节点的指针
}VertexNode;
...
void CreatAdjlist(AdjList* graph, int vertexnum, int edgenum)    //建立一个创建邻接表的函数并对邻接表进行初始化操作
{
    ...
    for (int i = 0; i < vertexnum; i++)    //通过 for 循环初始化每个顶点节点
    {
        ...
        graph->vertex[i].first = NULL;     //使顶点节点的指针初始化指向为空，如果有边则可以从空添加
    }
}
...
void CreatEdge(AdjList* graph, int begin, int dest)    //建立一个创建边节点的函数，获取边节点及其指向的节点
{
    ...
    newNode->next = graph->vertex[begin].first;        //如果对应的节点已经有指向的边，将新建的边节点的指针指向原有的第一个边节点
    graph->vertex[begin].first = newNode;              //使新建的边成为对应顶点指向的第一个边节点
}
...
void PrintAdjList(AdjList* graph)           //创建打印邻接表的函数
{
    for (int i = 0; i < graph->vertexnum; i++) //通过遍历顶点节点及其指向的边节点
    {
        ...
        EdgeNode* edge = graph->vertex[i].first;
